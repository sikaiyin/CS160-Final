...
public class TaskScreenActivity3Health2Examination extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_taskscreen3health2examination);
                                ...
                                documentReference.update("fHealth", Integer.toString(Integer.parseInt(document.get("fHealth").toString()) + 5))
                                        .addOnSuccessListener(new OnSuccessListener<Void>() {
                                            ...
                                            public void onSuccess(Void aVoid) {
                                                Toast.makeText(TaskScreenActivity3Health2Examination.this, "Updated Successfully",
                                                        Toast.LENGTH_SHORT).show();
                                            }
                                        }).addOnFailureListener(new OnFailureListener() {
                                    @Override
                                    public void onFailure(@NonNull Exception e) {
                                        Toast.makeText(TaskScreenActivity3Health2Examination.this, "Unable to Update",
                                                Toast.LENGTH_SHORT).show();
                                    }
                                }).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        Intent intent = new Intent(TaskScreenActivity3Health2Examination.this, LearningScreenActivity3Health2Examination.class);
                                        startActivity(intent);
                                    }
                                });
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()){
                            ...
                            if(document.exists()){
                                currentLabel=label2;
                                documentReference.update("fOption", FieldValue.arrayUnion(currentLabel.getText() + "\n At  " + currentTime.toString()));
                                documentReference.update("fCurrentBalance", FieldValue.arrayUnion(Integer.toString(Integer.parseInt(document.get("fBudget").toString()) - 120)));
                                documentReference.update("fBudget", Integer.toString(Integer.parseInt(document.get("fBudget").toString()) - 120));
                                documentReference.update("fAcademics", Integer.toString(Integer.parseInt(document.get("fAcademics").toString()) + 0));
                                documentReference.update("fSocial", Integer.toString(Integer.parseInt(document.get("fSocial").toString()) - 0));
